...
function buildBoardMarkup(sections){
  ...
  const markup=sections.map(section=>{
    ...
    const routes=(section.columns||[]).map(col=>{
      ...
      return `<section class="route${overlayView.hide_loaded&&routeStats.total>0&&routeStats.loaded===routeStats.total?' empty-route':''}"><div class="route-title"><strong title="${esc(col.location)}">${esc(col.location)}</strong><span>${esc(progress)}</span></div>${rows||'<div class="empty">Loaded</div>'}</section>`;
    }).join('');
    const totalLabel=mode==='aggregate_pickups'?`${section.total} SCU shared total`:`${section.total} SCU total`;
    return `<article class="group"><div class="group-head"><small>${esc(label)}</small><strong title="${esc(fixed)}">${esc(fixed)}</strong><span>${esc(totalLabel)}</span></div>${routes}</article>`;
  }).join('');
...
class OverlayController:
    ...
    def _finish_main_native_resize(self, width: int, height: int) -> None:
        if self.get_main_window_status().get("maximized"):
            return
        with self._lock:
            self.main_settings["width"] = max(1180, int(width))
...
.footer{height:28px;display:flex;align-items:center;justify-content:space-between;gap:12px;color:#777d97;font-size:9.5px;padding:0 2px;white-space:nowrap;overflow:hidden}.footer span{overflow:hidden;text-overflow:ellipsis}.greenText{color:var(--green)!important}.blueText{color:var(--blue)!important}.redText{color:var(--red)!important}.amberText{color:var(--amber)!important}
...
<div class="app">
  ...
  <section class="workspace-tools">
    <div class="first-run" id="onboarding" hidden><div><strong id="onboardingTitle">Choose a Game.log</strong><small id="onboardingCopy">Select a log file to populate the tracker.</small></div><div class="log-suggestions" id="logSuggestions"></div></div>
  </section>
  <div class="shell">
    <aside class="side">
...
$('logSuggestions').addEventListener('click',async event=>{const button=event.target.closest('button[data-path],button[data-command]');if(!button)return;if(button.dataset.path){$('logPath').value=button.dataset.path;await action('set_path');await action('scan')}else if(button.dataset.command==='browse'){$('browseBtn').click()}else if(button.dataset.command==='scan'){await action('scan')}});
...
function updateOnboarding(data){const panel=$('onboarding'),suggestions=$('logSuggestions');const hasPath=!!String(data.log_path||'').trim();const hasRows=Number(data.row_count||0)>0;const show=!hasPath||(!hasRows&&!data.watching);panel.hidden=!show;if(!show)return;$('onboardingTitle').textContent=hasPath?'No contracts in view':'Choose a Game.log';$('onboardingCopy').textContent=hasPath?'Scan the selected log or start watching for new hauling contracts.':'Use a detected log path or browse to your Star Citizen Game.log.';const current=String(data.log_path||'');const guessed=(data.guessed_log_paths||[]).filter(path=>path&&path!==current).slice(0,2);const buttons=guessed.map(path=>`<button class="suggestion-btn" type="button" data-path="${esc(path)}" title="${esc(path)}">${esc(path.split(/[\\\\/]/).pop()||'Game.log')}</button>`);buttons.push(`<button class="suggestion-btn" type="button" data-command="browse">Choose Log</button>`);if(hasPath)buttons.push(`<button class="suggestion-btn" type="button" data-command="scan">Scan Log</button>`);suggestions.innerHTML=buttons.join('')}
